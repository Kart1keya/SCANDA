...
def err2str(code):
    if code == -1:
        ...
        return "DanS signature not found."
    elif code == -8:
...
def pprint_cmpids_ex(cmpids):
    output = ""
    output = output + ("{:>20s}{:>16s}{:>16s}{:>32s}{:>39s}".format("Compiler Patchlevel", "Product ID",
        "Count", "MS Internal Name", "Visual Studio Release\n"))

    cnt = 0
    for e in cmpids:
        output = output + ("{:>20s}{:>16s}{:>16s}{:>32s}{:>39s}".format(
            "{:5d}".format(e['mcv']),
            "0x{:04x}".format(e['pid']),
            "0x{:08x}".format(e['cnt']),
            prodids.int_names.get(e['pid'], 'prodidUtc1900_POGO_O_CPP') if have_pids else '<unknown>',

            "{:18s} ({})\n".format(*prodids.vs_version(e['pid'])) if have_pids else '<unknown>'))
        print e['pid']
...
def pprint_header_ex(rich):
    output = pprint_cmpids_ex(rich['cmpids'])
    if rich['csum_calc'] == rich['csum_file']:
        output = output + ("Checksums match! (0x{:08x})\n".format(rich['csum_calc']))
    else:
        output = output + ("Checksum corrupt! (calc 0x{:08x}, file "
        "0x{:08x})\n".format(rich['csum_calc'], rich['csum_file']))
    return output
